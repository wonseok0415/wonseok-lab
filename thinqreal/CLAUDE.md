...
- **(후속) 16:17 재시험: L3 2건 모두 통과 (combo Δ-0.201 / away Δ+0.613, 둘 다 방향 반대 note = 오후 태양 조건 일관)**. 남은 실패 1건 = combo L2 — 판독: 물리 증거상 실행됐고(직후 외출 모드가 Δ+0.613로 "끌 것"이 있었음), post 창의 "죄송해요, 지금 요청하신 작업을 실행할 수는 없어요"는 이미 실행된 뒤 확답에 대한 반응(기존 "무슨 말씀…" 계열)으로 추정. 첫 창 전사가 "2."로 뭉개져 기대 키워드 미검출이 직접 원인 — 첫 창 녹음 청취로 실제 발화 확인 대기(전사 실패 vs 짧은 확답 스타일 구분). L3 latency 0ms 문제(촬영 전 변화 완료를 0ms로 기록 → 통계 오염)는 반응 시간 미측정(None)+note로 수정, selftest 29건 통과
