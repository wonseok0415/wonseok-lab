...
- **발표자료 4장 추가** (팀장·팀원 요청 — L3 픽셀 판정 원리·알고리즘·구조): ①구축 3단계 개요(말→행동, API 대신 카메라, AI 없이 산수) ②판정 원리(카메라 화면 목업 + 대상/참조 영역 + 상대값 공식 + 자동 노출 함정 실증) ③판정 알고리즘(절대 임계값→Δ→|Δ| 3단 진화 표 — "실측→수정→재실측" 서사) ④점검 시퀀스(6단계 + 무조건 확답 비대칭 + 증거 보존 원칙). 파일 `FieldCheck_진행보고_20260805.pptx`(구 20260802 대체), 원본은 PROGRESS_REPORT.md §9 신설(§9~13 재번호). **실측 스냅샷 사진을 받으면 ②의 목업을 실제 화면으로 교체 예정** — 사용자가 recordings/camera/의 jpg 2장(켜짐/꺼짐)을 저장소 fieldcheck/images/에 커밋해 주면 됨
