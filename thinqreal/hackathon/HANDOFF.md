...
| 파일 | 용도 |
|---|---|
| `structure_diagram_860px.jpg` (860×877) | 웹폼 규격 제출용 |
| `structure_diagram_1720px.jpg` (1720×1754) | 2배 해상도 원본 — 폼이 자동 축소를 지원하면 이쪽 업로드 권장, 발표 자료 재사용용 |
...
- 1차본이 글자가 작아 가독성 지적받음 → 폰트 전면 확대(박스 제목 13px 등).
- **심사기준 기반 피드백 반영 (2026-08-27, 최종 확정본)** — "구조도는 강점이나 모두 구현된 것처럼 보임 → 구현 상태 구분 필요":
  - 모든 박스 우측 상단에 **구현 상태 배지 4종**: 기존 활용(올리브 채움 — 예약·설문·월간 리포트·ROI) /
    시범 운영(올리브 테두리 — ThinQ ON 응답 기록·웹캠 가전 동작·품질 판정 Agent·이상 알림, FieldCheck 기반) /
    해커톤 구현(검정 채움 — 현장 대화 녹음·전처리 4종·방문 준비 Agent·현장 인사이트 Agent·브리핑·인사이트·액션 아이템) /
    향후 확대(점선 — 범례에만: 가전 실제 동작 검증 고도화·타 쇼룸 적용).
  - Core 상단에 "4-Agent 오케스트레이션 = 해커톤 구현" 배지 — 핵심 신규 구현 명시.
  - 흐름도 아래 **운영 규칙 밴드 4항**: ① Agent 결과는 원문·발생 시점·예약 ID로 근거 연결 ② 저신뢰 결과는 자동 확정 없이 검토 표시
    ③ 품질 이상은 응답 기록·가전 동작 결과 교차 확인 ④ 리포트는 담당자 승인 후 발행.
  - 그 아래 **구현 상태 범례** 4칸 + 기존 Agent 색 범례·데이터 키 유지.
