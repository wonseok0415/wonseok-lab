...
| 단계 | 무엇을 보는가 | 어떻게 |
|------|--------------|--------|
| **L1** | 응답을 했는가 / 몇 초 걸렸는가 | 녹음의 A-가중 음성 대역 레벨 + 발성 비율 (2026-07 ATOM TTS 장애가 이 단계에서 잡힘) |
...
- ⚠ **첫 실행 시 모델 파일(약 500MB, `small` 기준)을 인터넷에서 한 번 내려받습니다.** 이후에는 캐시를 재사용하므로 인터넷이 필요 없습니다. 리그를 설치할 곳의 네트워크가 제한적이라면, **인터넷이 되는 곳에서 `--transcribe`를 한 번 실행해 모델 캐시를 만든 뒤** 리그로 옮기세요.
...
리그는 **소리를 내어** 말을 겁니다. 방문객 시연 중에 점검 발화가 나가면 시연을 방해하므로, 발화 직전에 예약 관리 시스템에 오늘 예약을 물어보고 예약 시간대에는 건너뜁니다.
...
> **원리**: 휴대폰 앱 값과 리그 값은 **둘 다 A-가중이 적용된** dB입니다. 차이는 기준점뿐 —
> 앱은 음압(20µPa) 기준, 리그는 마이크 신호의 디지털 최대치 기준. dB는 비율 단위라
