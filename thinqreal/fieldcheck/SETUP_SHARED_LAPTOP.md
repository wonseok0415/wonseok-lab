...
- [ ] **A1. 운영 전용 로컬 계정 생성** — 기증자 개인 계정은 제거/비활성. 계정 이름 예: `thinqreal`,
      로그인 암호 설정 (관리자 3인 공유)
- [ ] **A2. 디스크 암호화 켜기** — 도난 시 디스크를 뽑아가도 내용(녹음 파일 등)을 못 읽게 하는 조치.
      로그인 암호와는 별개다 (아래 참고 박스).
      - Windows **Pro**: 제어판 → BitLocker 드라이브 암호화 → 켜기
      - Windows **Home**: 설정 → 개인 정보 및 보안 → **장치 암호화** → 켜기 (메뉴가 없으면 TPM 미지원
        기종 — 이 항목은 보류하고 대신 A4 보관 최소화를 더 엄격히)
      - **복구 키(48자리)는 반드시 별도 보관** — 인쇄 1부를 사무실에, 파일 1부를 개인 Drive에.
        복구 키를 잃으면 Windows 재설치 외엔 데이터 접근 불가. 장비 안에만 저장하는 것은 금지
