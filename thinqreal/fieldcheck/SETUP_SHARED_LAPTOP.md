...
- [ ] **A1. 운영 전용 로컬 계정 생성** — 기증자 개인 계정은 제거/비활성. 계정 이름 예: `thinqreal`,
      로그인 암호 설정 (관리자 3인 공유)
- [x] **A2. 디스크 암호화 → 불필요 판정으로 종결 (2026-08-21)** — 이 장비는 **Windows Home +
      2013년 기종(TPM 미지원)이라 BitLocker·장치 암호화 모두 불가**. 단 쇼룸이 **도어락 +
      관리자 동행 출입 통제**로 확인되어(사용자) 도난 시나리오가 사실상 닫힘 → 물리 잠금
      (켄싱턴 락)·당일 파기 등 보완 조치는 **생략**. 녹음 보관은 FieldVoice 원 정책(30일) 유지.
      FC_API_KEY는 유출 시 Script Property에서 즉시 교체 가능한 저위험 — 감수.
      전제가 바뀌면(출입 통제 완화·장비 반출 상시화) 재검토
