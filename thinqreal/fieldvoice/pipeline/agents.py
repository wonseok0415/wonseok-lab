...
# ── 에이전트 5: 리포트 편집 (1페이지 요약 — 관리자 페이지 적재용 포맷) ──

def compile_report(insights_md, summary_md, context_md, cfg):
    system = [
        _bg_block(),
        {
            "type": "text",
            "text": """임무: 아래 분석 결과 3종을 바쁜 운영자가 2분 안에 읽는 1페이지 리포트로 편집하라.

규칙:
- 전체 60줄·공백 제외 900단어 이내. 이 리포트는 관리자 대시보드에 쌓여 회차끼리
  비교되므로 구조를 정확히 지켜라.
- 새 분석을 하지 말 것 — 이미 도출된 내용에서 고르고 압축만 한다.
- 인용은 항목당 최대 1개, 한 줄로 자른다.

출력 구조 (이 순서 고정):
## 한 줄 결론
## 핵심 인사이트 TOP 5
(표: | 인사이트 | 근거 한 줄 | 신뢰도 | — 카테고리 균형보다 가치 순)
## 지금 바꿀 것 3가지
(운영·시연·질문 프레임 개선 제안 중 실행 가능성 높은 순 3개, 각 2줄 이내)
## 다음 회차 검증 가설
(2~3개, 각 한 줄 + 검증 질문 한 줄)
## 읽을 때 주의
(표본·전사 품질 한계 2줄 이내)""",
        },
    ]
    ...
    return _call(system, user, cfg, max_tokens=16000)
